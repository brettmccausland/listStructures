...
#define Rightchild(n) (size_t(n * 2 + 1))
//---------------- CURENT TASK ---------------
// insert and delete
//
//
...
class bst
{
    public:
        ...
        // bst(BST_TRAVERSAL_TYPES  t = IN_ORDER);
         bst();
         T getmax()const;
         bst<T>& copy(const bst<T> &other);
         ~bst();
          bst<T>& operator=(const bst<T> &other);
          void insert(const T d, size_t root = NULL);
          void print(size_t root=NULL);//preorder
...
//        node<T>* getroot()const;
    ...
    private:
     ...
     //
        ...
        //fptr whatToDo[3]; // nothing more than ouput functions
...
//}
template<typename T>
void bst<T>::print(size_t root)//preorder
{
    if(root<= Lastentered)
    {
        cout<<DataV[root]<<",";
        print(Leftchild(root));
        print(Rightchild(root));
    }
}
...
template<typename T>
bst<T>& bst<T>::copy(const bst<T> &other)
{
    if(this != &other)
    {
        this->DataV.clear();
        ...
        for(int i= 0;i<other.DataV.size();++i)
            this->DataV.push_back(other.DataV[i]);
    }
    return *this;
}
template<typename T>
void bst<T>::insert(const T d, size_t root)
{
     if (DataV[root]==NULL)
        if(DataV.emplace(root,d))

    size_t whichchild;
    if(root)
        DataV[root];
   else
    whichchild= DataV[root]>d ? Leftchild(root):Rightchild(root);
    insert(d,whichchild);
 }
////template<typename T>
